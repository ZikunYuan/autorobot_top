...
#define ANO_DT_CTRL_TYPE      (float)   //发送到上位机的参数类型
...
void u1_putbuff(u8* msg,u8 len);       //通过UART1发送
void u4_putbuff(u8* msg,u8 len);       //通过UART1发送
...
//			  u8 Res;
        uint8_t data = 0;
        data = USART_ReceiveData(ANO_DT_UART_PORT);
//        static uint8_t _data_len = 0;
//        static uint8_t state = 0;
		ANO_DT_Data_Receive_Prepare(data);
		/*Flag_ANO_DT_Data = (Flag)ANO_DT_Data_Receive_Prepare((uint8_t)data);*/
...
#if defined(USE_PID_01)
    //PID1
    PID_01_P = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID1].kp);
    PID_01_I = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID1].ki);
    PID_01_D = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID1].kd);
...
#if defined(USE_PID_02)
    //PID2
    PID_02_P = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID2].kp);
    PID_02_I = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID2].ki);
    PID_02_D = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID2].kd);
...
#if defined(USE_PID_03)
    //PID3
    PID_03_P = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID3].kp);
    PID_03_I = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID3].ki);
    PID_03_D = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID3].kd);
#endif

#if defined(USE_PID_04)
    //PID4
    PID_04_P = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID4].kp);
    PID_04_I = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID4].ki);
    PID_04_D = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID4].kd);
#endif

#if defined(USE_PID_05)
    //PID5
    PID_05_P = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID5].kp);
    PID_05_I = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID5].ki);
    PID_05_D = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID5].kd);
#endif

#if defined(USE_PID_06)
    //PID6
    PID_06_P = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID6].kp);
    PID_06_I = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID6].ki);
    PID_06_D = ANO_DT_PARAMETER_TYPE((double)ctrl_1.PID[PID6].kd);
#endif
...
void u1_putbuff(uint8_t* msg,uint8_t len)       //通过UART1发送
{
    uint8_t t=0;
    for(t=0; t<len; t++)
    {
...
void u4_putbuff(uint8_t* msg,uint8_t len)       //通过UART4发送
{
    uint8_t t=0;
    for(t=0; t<len; t++)
    {
        USART_SendData(UART4, msg[t]);//向串口1发送数据
